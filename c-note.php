        ...
        <ul class="navbar-nav mr-auto">
          <li class="nav-item dropdown">
            <a class="nav-link" href="#" id="dropdown01" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Notifications 
                ...
                $query = "SELECT * from `notifications` where `status` = 'unread' && `email`='$user_check' order by `date` DESC";
            ...
            <div class="dropdown-menu" aria-labelledby="dropdown01">
                <?php
                 $user_check=$_SESSION['login_user'];
                $query = "SELECT * from `notifications` WHERE `email`='$user_check' order by `date` DESC";
                 if(count(fetchAll($query))>0){
